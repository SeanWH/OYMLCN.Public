...
namespace OYMLCN.Open.SpecialTest
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            //var keyText = new OYMLCN.Word.KeyWord.TextRankExtractor();
            //var idf = new OYMLCN.Word.KeyWord.IdfLoader();
            ...
            //tokens = segmenter.Tokenize(s, TokenizerMode.Search);
            //foreach (var token in tokens)
            //    Console.WriteLine("word {0,-12} start: {1,-3} end: {2,-3}", token.Word, token.StartIndex, token.EndIndex);

            //var posSeg = new PosSegmenter();
            //s = "一团硕大无朋的高能离子云，在遥远而神秘的太空中迅疾地飘移";
            //var posTokens = posSeg.Cut(s);
            //Console.WriteLine(string.Join(" ", posTokens.Select(token => string.Format("{0}/{1}", token.Word, token.Flag))));
            ...
            //text = @"在数学和计算机科学/算学之中，算法/算则法（Algorithm）为一个计算的具体步骤，常用于计算、数据处理和自动推理。精确而言，算法是一个表示为有限长列表的有效方法。算法应包含清晰定义的指令用于计算函数。
            ...
            //keywords = extractor.ExtractTags(text, 10, Constants.NounAndVerbPos);
            //foreach (var keyword in keywords)
            //    Console.WriteLine(keyword);

            //while (true)
            //{
            //    var check = new SpellChecker();
            //    var data = check.Suggests(Console.ReadLine());
